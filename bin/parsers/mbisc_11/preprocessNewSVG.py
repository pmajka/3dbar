...
TRANSF_MTX_FRONT = "0.0235184448749, -1.07547496568, -0.0235183443086, 19.1613593603" 
TRANSF_MTX_BACK = "0.0235184951584, -1.06628153349, -0.0235185023418, 19.1636954959" 
...
def parseSVG(srcFilename, dstPattern):
    ...
    slideNodes = [(i, node, TRANSF_MTX_FRONT if i == 0 else TRANSF_MTX_BACK)\
                   for (i, node) in enumerate(slideNodes) if i < 1 or i > 51]
    ...
    for i, node, mtx in slideNodes:
        cleanSlide(node, slideTemplate)
        transformationNode.setAttribute('content', mtx)
        #getTags(node, i)
