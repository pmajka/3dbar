...
def _FindExtremeLabels(svgdoc):
    ...
    xCoordList= sorted([int(i[2]) for i in TextNodes if i[1]==TopMost])
    ...
    #   print "grid"          
        ...
        if _isGridLine(el):
            ...
            if el.attributes['y1'].value==el.attributes['y2'].value and\
                float(el.attributes['x2'].value)-float(el.attributes['x1'].value)>CONF_GIRD_ELEMENT_MIN_LENGTH:
            ...
            if el.attributes['x1'].value==el.attributes['x2'].value and\
                float(el.attributes['y2'].value)-float(el.attributes['y1'].value)>CONF_GIRD_ELEMENT_MIN_LENGTH:
                VerticalGrid.append(float(el.attributes['x1'].value))   

    #   print HorizontalLines
    #   print VerticalLines
    # TODO: Reverse elements in list below
...
def _cleanSVG(svgdoc):
    ...
    for el in svgdoc.getElementsByTagName('text'):
        ...
        # The last thing that we want to do is change the colour of lasting
        # elements to black as it is more convinient to notice them at black
        # background. Code below assumes that colour is assigned using fill
        # attrubute (without using style notation). Color which is used is
        # defined by CONFIG_ARROW_LINE_STROKE
        el.setAttribute('fill', CONFIG_ARROW_LINE_STROKE)
