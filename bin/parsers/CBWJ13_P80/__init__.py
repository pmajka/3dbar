...
class AtlasParser(bar.barBitmapParser):
    ...
    def __init__(self, inputDirectory, outputDirectory, **kwargs):
        ...
        # source dataset
        sourceFilename = 'pnd80_average_labels.nii'
        ...
        structureColours = getDictionaryFromFile(fullnameMappingFile, 1, 3)
        ...
        self._defineSlideRange(antPostAxis=1)
        self._pathNumber = 0
    ...
    def parse(self, slideNumber):
        ...
        # too large
        map(lambda x: \
                x._attributes.update({'font-size':'16px'}),\
                tracedSlide.labels)
    ...
    def reindex(self):
        ...
        #Load hierarchy from parents.txt - not in this template.
        ...
        fullnameMappingFile = os.path.join(self.inputDirectory,'fullnames.txt')
        self.indexer.setNameMappingFromFile(fullnameMappingFile, 1, 2)

        self.writeIndex()

    def _getSourceImage(self, slideNumber):
        volumeSlide = self._volume[:, :, self.slideRange[-1] - \
                                   self.slideRange[slideNumber]]
    ...
    def _getZCoord(self, slideNumber):
        coronal_coordinates = \
            ((self.slideRange[-1] - coronal_origin_voxel_index) \
            - slideNumber) * voxelSize
        return coronal_coordinates
...
if __name__=='__main__':
    try:
        ...
        inputDirectory  = 'atlases/CBWJ13_P80/src/'
        outputDirectory = 'atlases/CBWJ13_P80/caf/'
